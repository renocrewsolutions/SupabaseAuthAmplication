/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { ClientWhereUniqueInput } from "../../client/base/ClientWhereUniqueInput";
import {
  ValidateNested,
  IsOptional,
  IsString,
  MaxLength,
} from "class-validator";
import { Type } from "class-transformer";
import { ClientUpdateManyWithoutFreelancersInput } from "./ClientUpdateManyWithoutFreelancersInput";
import { ContractUpdateManyWithoutFreelancersInput } from "./ContractUpdateManyWithoutFreelancersInput";
import { ProposalUpdateManyWithoutFreelancersInput } from "./ProposalUpdateManyWithoutFreelancersInput";

@InputType()
class FreelancerUpdateInput {
  @ApiProperty({
    required: false,
    type: () => ClientWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => ClientWhereUniqueInput)
  @IsOptional()
  @Field(() => ClientWhereUniqueInput, {
    nullable: true,
  })
  client?: ClientWhereUniqueInput | null;

  @ApiProperty({
    required: false,
    type: () => ClientUpdateManyWithoutFreelancersInput,
  })
  @ValidateNested()
  @Type(() => ClientUpdateManyWithoutFreelancersInput)
  @IsOptional()
  @Field(() => ClientUpdateManyWithoutFreelancersInput, {
    nullable: true,
  })
  clients?: ClientUpdateManyWithoutFreelancersInput;

  @ApiProperty({
    required: false,
    type: () => ContractUpdateManyWithoutFreelancersInput,
  })
  @ValidateNested()
  @Type(() => ContractUpdateManyWithoutFreelancersInput)
  @IsOptional()
  @Field(() => ContractUpdateManyWithoutFreelancersInput, {
    nullable: true,
  })
  contracts?: ContractUpdateManyWithoutFreelancersInput;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  email?: string | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  name?: string | null;

  @ApiProperty({
    required: false,
    type: () => ProposalUpdateManyWithoutFreelancersInput,
  })
  @ValidateNested()
  @Type(() => ProposalUpdateManyWithoutFreelancersInput)
  @IsOptional()
  @Field(() => ProposalUpdateManyWithoutFreelancersInput, {
    nullable: true,
  })
  proposals?: ProposalUpdateManyWithoutFreelancersInput;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  skills?: string | null;
}

export { FreelancerUpdateInput as FreelancerUpdateInput };
